...
function wxpay(){
    ...
    $.ajax({
        type: "post",
        url: '/edu/api/doSign?randomParam='+Math.random(),
        cache:false,
        async:false,
        data:json,
...
function countActualAmount(){
	...
	var re=	/^\d{0,6}$/;//判断输入的值小于6位数
    if(re.test(money)){
		console.info("bbbbbb==",money.substring(0,money.length-1));

    }else{
		console.info("aaaaaaaaaa==",money.substring(0,money.length-1));
