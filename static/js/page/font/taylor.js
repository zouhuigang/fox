...
;(function(_) {
	...
	var TaylorChar = function(id, opts, xml) {
		this.id = id;
		...
		var tpl = '<div class="TaylorChar-wrapper"><canvas class="TaylorChar-canvas"></canvas></div>';
		tpl = '<div class="TaylorChar-controls"></div>' + tpl;
		this.el.innerHTML = tpl;

		this.dom = {
			canvas: this.el.querySelector(".TaylorChar-canvas"),
			canvasWrapper: this.el.querySelector(".TaylorChar-wrapper"),
			controls: this.el.querySelector(".TaylorChar-controls")
		};
		this.preloadImg();
		this.initControls();
	...
	TaylorChar.prototype = {
		
		constructor: TaylorChar,


		mergeOptions: function(opts) {
			opts = _.assign({}, TaylorChar.defaultOpts,this.opts,opts);
			return opts;
		},
		...
		initControls: function() {
			this.controls = {};
			if(!this.opts.controls.length || !TaylorChar.Control) return false;
			for(var i = 0; i < this.opts.controls.length; i++) {
				var c = null;
				if(typeof this.opts.controls[i] == "string") {
					c = new window['TaylorChar']['Control'][this.opts.controls[i]](this);
				}else if(typeof this.opts.controls[i] == "object"){
					for(var controlName in this.opts.controls[i]) break;
					c = new window['TaylorChar']['Control'][controlName](this, this.opts.controls[i][controlName]);
				}
				if (c) {
	...
	TaylorChar.Control = function(drawingBoard, opts){
		this. board = drawingBoard;
		this.opts = _.extend({}, this.defaults, opts);
		this.el = document.createElement('div');
		this.el.classList.add('TaylorChar-control');
		if(this.name){
			this.el.classList.add('TaylorChar-control-'+this.name+'s');
		}
		this.initialize.apply(this, arguments);
		// return this;
	};

	TaylorChar.Control.prototype = {
		name: '',
	...
	TaylorChar.Control.extend = function(protoProps, staticProps){
		var parent = this;
		var child;
	...
	TaylorChar.Control.DisplayMode = TaylorChar.Control.extend({
		name: 'displaymode',
		...
		initialize: function() {
			...
			["brush", "skeleton", "strokeColor", "animation", "uniformWidth"].forEach(function(element, index, array){
				if(element) {
					tpl = tpl + '<button class="TaylorChar-control-displaymode control-displaymode-'+element+'-button" data-mode="'+ element +'">'+ element+'</button> ';
				}
			});
			this.el.innerHTML = tpl;
	...
	TaylorChar.Control.Navigation = TaylorChar.Control.extend({
		name: 'navigation',
		...
		initialize: function(){
			...
			if(this.opts.imgRadio) {
				tplRadio = tplRadio + '<div class="TaylorChar-navigation-radio"><button class="TaylorChar-navigation-radio-blank" data-radio="blank">blank</button><button class="TaylorChar-navigation-radio-grid" data-radio="grid">grid</button></div>';
				div.innerHTML = tplRadio;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.reset) {
				tplReset = tplReset + '<button class="TaylorChar-control-navigation-reset">&times;</button>';
				div.innerHTML = tplReset;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.add) {
				tplAdd = tplAdd + '<button class="TaylorChar-control-navigation-add">addChar</button>';
				div.innerHTML = tplAdd;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.update) {
				tplUpdate = tplUpdate + '<button class="TaylorChar-control-navigation-update">update</button>';
			...
			// });
			this.el.getElementsByClassName("TaylorChar-control-navigation-reset")[0].addEventListener('click', function(){
	...
	TaylorChar.Control.Size = TaylorChar.Control.extend({
		name: 'size',
		...
		initialize: function() {
			...
			// }
			var buttonArray = Array.prototype.slice.call(this.el.querySelectorAll('.TaylorChar-control-size input'),0);
		...
		_rangeTemplate: function(){
			...
			["wmax", "wmin", "timeScale"].forEach(function(element, index, array){
				if(element) {
					tpl = tpl + '<div class="TaylorChar-control-size TaylorChar-control-size-'+ element +'" >'
					+'<div class="TaylorChar-control-inner" title="'+ that.opts[element].size +'">'
					+'<span>'+element+'</span>' 
					+'<input type="range" min="'+ that.opts[element].min +'" max="'+ that.opts[element].max +'" value="'+ that.opts[element].size 
					+'" step="'+that.opts[element].step+'" class="TaylorChar-control-size-range-input" data-range="'+ element +'">' 
					+ '<span class="TaylorChar-control-size-range-current"></span>' 
					+'</div></div>'
				}
			});
			this.el.innerHTML = tpl;
		...
		_dropdownTemplet: function(){
			...
			["gaussian", "wmax", "wmin", "timeScale"].forEach(function(element, index, array){
				if(element) {
					tpl = tpl + '<div class="TaylorChar-control-size-'+ element +'">'
					+'<div class="TaylorChar-control-inner" title="'+ that.opts[element].size +'">' 
					+'<input type="dropdown" min="'+ that.opts[element].min +'" max="'+ that.opts[element].max +'" value="'+ that.opts[element].size 
					+'" step="'+that.opts[element].step+'" class="TaylorChar-control-size-dropdown-input">' 
					+ '<span class="TaylorChar-control-size-dropdown-current"></span>' 
					+'</div></div>'
				}
			});
			this.el.innerHTML = tpl;
	...
	TaylorChar.Control.Curve = TaylorChar.Control.extend({
		name : 'Curve',
...
// TaylorChar.Control.File = TaylorChar.Control.extend({
// 	name: "File",
// 	default: {
