...
;(function(_) {
	...
	Character.prototype = {
		...
		_addStroke: function() {
			...
			console.info("字的长度",this.strokeArray);
			
			this.isDrawing = false; 
			this.r = 0;
		},
		_GetAllValue:function(){
	...
	console.log("sdsad");
			...
			if(this.opts.xmlFile){
				...
				this.mode = "brush";
				this.parseXML();
				//console.info("解析xml文件",this.strokeArray);
				this.changeXY(this.strokeArray, this.opts  , 1); //变更坐标位置
				this.drawAllPoints(this.canvas, this.strokeArray);
			}
			if(xml){
				xml.addEventListener("change",function(){
					...
					console.info("解析xml结束");
					self.changeXY(self.strokeArray, self.opts, 1);//变更坐标位置
					self.drawAllPoints(self.canvas, self.strokeArray);//将坐标点绘制到画布中
					//console.info("监听文件改变，读取并解析xml文件",self.strokeArray);
				},false);
			}
		...
		drawAnimation: function(){
			...
			if(this.strokeArray.length != 0){
				console.info("写字过程动画显示中..",this.strokeArray);	
				var intervalID;
				var i = 0;
				var j = 1;
				var points = this.strokeArray[i];
		...
		changeXY: function(strokeArray, opts, charScale, flag) {
			var charAspectRatio;
			//console.info("正在适应画布..",strokeArray,opts,charScale,flag);
	...
	Character.Control.DisplayMode = Character.Control.extend({
		...
		initialize: function() {
			var that = this;
			var tpl = '';
			var nameArray=["刷子", "骨架", "笔画色彩", "动画", "宽度均匀"];
			["brush", "skeleton", "strokeColor", "animation", "uniformWidth"].forEach(function(element, index, array){
				if(element) {
					tpl = tpl + '<button class="character-control-displaymode control-displaymode-'+element+'-button" data-mode="'+ element +'">'+ nameArray[index]+'</button> ';
				}
			});
			this.el.innerHTML = tpl;
	...
	Character.Control.Navigation = Character.Control.extend({
		...
		initialize: function(){
			...
			if(this.opts.imgRadio) {
				tplRadio = tplRadio + '<div class="character-navigation-radio"><button class="character-navigation-radio-blank" data-radio="blank">空白</button><button class="character-navigation-radio-grid" data-radio="grid">网格</button></div>';
				div.innerHTML = tplRadio;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.reset) {
				tplReset = tplReset + '<button class="character-control-navigation-reset">清空</button>';
				div.innerHTML = tplReset;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.add) {
				tplAdd = tplAdd + '<button class="character-control-navigation-add">添加文字</button>';
				div.innerHTML = tplAdd;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.update) {
				tplUpdate = tplUpdate + '<button class="character-control-navigation-update">更新</button>';
				div.innerHTML = tplUpdate;
				this.el.appendChild(div.firstChild);
			}	

			//得到xml数据
			tplXml = tplXml + '<button class="character-control-navigation-xml">生成xml</button>';
			div.innerHTML = tplXml;
			this.el.appendChild(div.firstChild);

			tplGif = tplGif + '<button class="character-control-navigation-gif">生成gif图</button>';
			div.innerHTML = tplGif;
			this.el.appendChild(div.firstChild);
	...
	Character.Control.Size = Character.Control.extend({
		...
		_rangeTemplate: function(){
			...
			var tpl = '';
			var that = this;
			var nameArray =	["高斯系数", "笔画最大宽度", "笔画最小宽度", "时间刻度"];
