...
;(function(_) {
	...
	Character.prototype = {
		...
		_setPointWidth: function(v, r, points) {
			...
			// 	a = this.opts.wmax/this.flatCurve(0, this.opts.flat);
			// 	v2 = this.flatCurve(v, this.opts.flat);
			// 	console.log("ha");
			// }else if (this.widthMethod === "sigmoidCurve") {
			// 	a = this.opts.wmax/this.sigmoid(0, this.opts.sigmoid);
			...
			var gaussian = this.opts.gaussian;
			w = this.opts.wmax*Math.pow(Math.E, -v*v/(2*gaussian*gaussian));
			// console.log(w);
			if(w < this.opts.wmin){
				w = this.opts.wmin;
			}
		...
		updateStrokeArray : function() {
			...
			for(i = 0; i< this.strokeArray.length; i++){
				for(j=1; j<= this.strokeArray[i].length; j++){
					...
					points[j-1].w = this._setPointWidth(v, j, points, p);
				}
			}
		},
			...
			for(i=0; i<stroke.length; i++) {
				...
				for(j=0; j<stroke[i].childNodes.length; j++){
					  ...
					  if(j == 0){
						...
						var w = this._setPointWidth(v, j+1, this.points,p);
	...
	Character.Control.Curve = Character.Control.extend({
		...
		initialize: function() {
			...
			divRange.innerHTML = 'gaussian<input type="range" min="0.1" max="3" value="1.3" step="0.1" class="character-control-curve-range-input" data-range="gaussian" />';
			...
			curveSelect.addEventListener('change', function(e){
				...
				if(value === "sigmoidCurve") {
					...
					divRange.innerHTML = 'flatCurve<input type="range" min="0.01" max="3" value="1" step="0.01" class="character-control-curve-range-input" data-range="flat" />';
