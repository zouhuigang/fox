//Õâ¸öÎÄ¼þ½ö½öÌá¹©Ð´×ÖµÄ¶ÔÏó
//ÒýÈëLoDash  _  ,ÎªÁËÊ¹ÓÃextend/assign·½·¨

;(function(_) {



	var AccChar = function(id, opts, xml) {
		this.id = id;
		...
		var tpl = '<div class="AccChar-wrapper"><canvas class="AccChar-canvas"></canvas></div>';
		tpl = '<div class="AccChar-controls"></div>' + tpl;
		this.el.innerHTML = tpl;

		this.dom = {
			canvas: this.el.querySelector(".AccChar-canvas"),
			canvasWrapper: this.el.querySelector(".AccChar-wrapper"),
			controls: this.el.querySelector(".AccChar-controls")
		};
		this.preloadImg();
		this.initControls();
		...
		this.init();
		//Õâ¸öÊÇ¸ö¶¯Ì¬µÄ£¬ ÏÈÕâÑù´¦Àí£¬ ÓÐ¸üºÃµÄ·½·¨ÔÙÐÞ¸Ä
	...
	AccChar.prototype = {
		...
		mergeOptions: function(opts) {
			opts = _.assign({}, AccChar.defaultOpts,this.opts,opts);
			return opts;
		},
		...
		/**
		 * È¡µÃÒ»¸öµãµÃËùÓÐ²ÎÊý£¬ÒÔ¶ÔÏóµÄ·½Ê½´æ´¢¡£
		 * @param  {Number} x      ±Ê´¥µÄµ±Ç°Î»ÖÃ
		 * @param  {Number} y      ±Ê´¥µÄµ±Ç°Î»ÖÃµÄy×ø±ê
		 * @param  {Array} points µ±Ç°µÄ±Ê»­Êý×é£¬²»¶Ï¸üÐÂµ½»­Íê£¬ÐèÒªÒÀ¿¿Ç°ÃæµÄµã¼ÆËãµÃµ½
		 * @return {Object}        ±Ê´¥µÄµ±Ç°µã
		 */
		...
		_setPointWidthByV: function(v, a, aa, aaa, r, points, deltaTime) {
			var m, v2, w;
			...
			if(r == 1){
				...
				var preWidth = points[r-2].w;
				if(this.widthMethod === "gaussianCurve"){

					// m = this.opts.wmax/this.gaussian(0, this.opts.gaussian);
					// v2 = this.gaussian(v, this.opts.gaussian);
					var b = 0.4;
					var width1 = this.opts.wmax*Math.pow(Math.E, -v*v/(2*this.opts.gaussian*this.opts.gaussian));
					...
					var width2 = preWidth - a * deltaTime;
		...
		/**
		 * È¡µÃµ±Ç°µãµÃ¿í¶È£¬ÓÉµ±Ç°µãµÃËÙ¶ÈµÃµ½
		 * @param  {Number} v      µ±Ç°µãµÃËÙ¶È
		 * @param  {Number} r      µ±Ç°µãµÄË÷Òý
		 * @param  {Array} points ÒÑ»æÖÆ±Ê»­µÄÊý×é
		 * @return {Number}        µ±Ç°µãµÃ¿í¶È
		 */
		...
		_setPointWidth: function(v, a, aa,aaa, r, points, deltaTime) {
			var w1 = this._setPointWidthByV(v, a, aa, aaa, r, points, deltaTime);
			var w2 = this._setPointWidthByA(v, a, r, points, deltaTime);
			// var w3 = this._setPointWidthByAcc(v, a,aa,aaa, r, points, deltaTime)
			// console.log(w1);
			// console.log(w2);
			...
			if(this.mode === "animation"){
				...
				for(i = 0; i< strokeArray.length; i++){
					...
					for(j=1; j<= strokeArray[i].length; j++){
						...
						//ÕâÁ½ÐÐ´úÂëÊÇ¸Ä±ärangeºó£¬ ÏàÓ¦µÄ²ÎÊý±ä»¯¡£
						// if(){
		...
		saveImg: function(radio) {
			...
			// ÕâÖÖÇé¿ö»¹Ã»´¦ÀíºÃ£¬×ö¸ö±êÇ©£¬Ö®ºó´¦Àí
			else if(radio === "blank"){
				// var ctx = this.canvas.getContext("2d");
		...
		init: function() {
			...
			// document.body.addEventListener('touchmove', function (event) {event.preventDefault();}, false);//¹Ì¶¨Ò³Ãæ
			//µ±×ÖÊÇxml±ê×¼×ÖÊ±£¬²»ÄÜÊéÐ´
		...
		_drawStrokeSkeleton: function(ctx, r, points,color) {
			...
			var y = points[r-1].y;
			//Ã¿¸öµã»æÖÆÇ°¶¼ÐèÒªbeginPathºÍmoveTo, ·ñÔò±Ê»­µÄ×Ö²»»áÖ±¡£
			if(r == 1){
				ctx.beginPath();
				ctx.moveTo(x,y);
			...
			if(this.strokeArray.length != 0){
				...
				intervalID = setInterval(
					function() {
						...
						//ÆäÊµ¿ÉÒÔµ¥¶ÀÐ´Ò»¸öupdate²ÎÊýµÄ£¬µ«ÊÇÒòÎª²»Çå³þº¯Êý¼ä´«²ÎµÄ¹æÂÉ£¬´ýÍê³É¡£
		...
		initControls: function() {
			this.controls = {};
			if(!this.opts.controls.length || !AccChar.Control) return false;
			for(var i = 0; i < this.opts.controls.length; i++) {
				var c = null;
				if(typeof this.opts.controls[i] == "string") {
					c = new window['AccChar']['Control'][this.opts.controls[i]](this);
				}else if(typeof this.opts.controls[i] == "object"){
					for(var controlName in this.opts.controls[i]) break;
					c = new window['AccChar']['Control'][controlName](this, this.opts.controls[i][controlName]);
				}
				if (c) {
	...
	AccChar.Control = function(drawingBoard, opts){
		this. board = drawingBoard;
		this.opts = _.extend({}, this.defaults, opts);
		this.el = document.createElement('div');
		this.el.classList.add('AccChar-control');
		if(this.name){
			this.el.classList.add('AccChar-control-'+this.name+'s');
		}
		this.initialize.apply(this, arguments);
		// return this;
	};

	AccChar.Control.prototype = {
		name: '',
	...
	AccChar.Control.extend = function(protoProps, staticProps){
		var parent = this;
		var child;
	...
	AccChar.Control.DisplayMode = AccChar.Control.extend({
		name: 'displaymode',
		...
		initialize: function() {
			...
			["brush", "skeleton", "strokeColor", "animation", "uniformWidth"].forEach(function(element, index, array){
				if(element) {
					tpl = tpl + '<button class="AccChar-control-displaymode control-displaymode-'+element+'-button" data-mode="'+ element +'">'+ element+'</button> ';
				}
			});
			this.el.innerHTML = tpl;
	...
	AccChar.Control.Navigation = AccChar.Control.extend({
		name: 'navigation',
		...
		initialize: function(){
			...
			if(this.opts.imgRadio) {
				tplRadio = tplRadio + '<div class="AccChar-navigation-radio"><button class="AccChar-navigation-radio-blank" data-radio="blank">blank</button><button class="AccChar-navigation-radio-grid" data-radio="grid">grid</button></div>';
				div.innerHTML = tplRadio;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.reset) {
				tplReset = tplReset + '<button class="AccChar-control-navigation-reset">&times;</button>';
				div.innerHTML = tplReset;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.add) {
				tplAdd = tplAdd + '<button class="AccChar-control-navigation-add">addChar</button>';
				div.innerHTML = tplAdd;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.update) {
				tplUpdate = tplUpdate + '<button class="AccChar-control-navigation-update">update</button>';
			...
			// });
			this.el.getElementsByClassName("AccChar-control-navigation-reset")[0].addEventListener('click', function(){
	...
	AccChar.Control.Size = AccChar.Control.extend({
		name: 'size',
		...
		initialize: function() {
			...
			// }
			var buttonArray = Array.prototype.slice.call(this.el.querySelectorAll('.AccChar-control-size input'),0);
		...
		_rangeTemplate: function(){
			...
			["wmax", "wmin", "timeScale"].forEach(function(element, index, array){
				if(element) {
					tpl = tpl + '<div class="AccChar-control-size AccChar-control-size-'+ element +'" >'
					+'<div class="AccChar-control-inner" title="'+ that.opts[element].size +'">'
					+'<span>'+element+'</span>' 
					+'<input type="range" min="'+ that.opts[element].min +'" max="'+ that.opts[element].max +'" value="'+ that.opts[element].size 
					+'" step="'+that.opts[element].step+'" class="AccChar-control-size-range-input" data-range="'+ element +'">' 
					+ '<span class="AccChar-control-size-range-current"></span>' 
					+'</div></div>'
				}
			});
			this.el.innerHTML = tpl;
		},
		//Õâ¸ö·½·¨ÔÝÊ±ÏÐÖÃ°É~  ÒÔºóÐèÒªÀ©Õ¹ÔÙÓÃ
		_dropdownTemplet: function(){
			var tpl = '';
			var that = this;
			["gaussian", "wmax", "wmin", "timeScale"].forEach(function(element, index, array){
				if(element) {
					tpl = tpl + '<div class="AccChar-control-size-'+ element +'">'
					+'<div class="AccChar-control-inner" title="'+ that.opts[element].size +'">' 
					+'<input type="dropdown" min="'+ that.opts[element].min +'" max="'+ that.opts[element].max +'" value="'+ that.opts[element].size 
					+'" step="'+that.opts[element].step+'" class="AccChar-control-size-dropdown-input">' 
					+ '<span class="AccChar-control-size-dropdown-current"></span>' 
					+'</div></div>'
				}
			});
			this.el.innerHTML = tpl;
	...
	AccChar.Control.Curve = AccChar.Control.extend({
		name : 'Curve',
...
// AccChar.Control.File = AccChar.Control.extend({
// 	name: "File",
// 	default: {
