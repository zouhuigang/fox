...
;(function(_) {
	...
	var BezierChar = function(id, opts, xml) {
		this.id = id;
		...
		var tpl = '<div class="BezierChar-wrapper"><canvas class="BezierChar-canvas"></canvas></div>';
		tpl = '<div class="BezierChar-controls"></div>' + tpl;
		this.el.innerHTML = tpl;

		this.dom = {
			canvas: this.el.querySelector(".BezierChar-canvas"),
			canvasWrapper: this.el.querySelector(".BezierChar-wrapper"),
			controls: this.el.querySelector(".BezierChar-controls")
		};
		this.preloadImg();
		this.initControls();
	...
	BezierChar.prototype = {
		
		constructor: BezierChar,


		mergeOptions: function(opts) {
			opts = _.assign({}, BezierChar.defaultOpts,this.opts,opts);
			return opts;
		},
		...
		draw: function(ctx, r, points, colorID, lineWidth) {
			...
			var image;
			var i;
			var tempX,tempY,tempW, tempV,tempAcc;
			var x1,x2,x3,x4,
					y1,y2,y3,y4,
					w1,w2,w3,w4;
			//三次贝塞尔曲线的控制点
        ...
        disAvg = dis34/(3.0 * dis24);
				...
				for(var u=0; u<sampleNumber; u++){
					...
					tempV = (1-t)*(1-t)*(1-t)*v2
									...
									+t*t*(1-t)*3*b2.v
									+t*t*t*v3;
					tempAcc = (1-t)*(1-t)*(1-t)*acc2
									+t*(1-t)*(1-t)*3*a2.acc
									+t*t*(1-t)*3*b2.acc
									+t*t*t*acc3;
					// tempW = this.opts.wmax*Math.pow(Math.E, -tempV*tempV/(2*this.opts.gaussian*this.opts.gaussian));
					tempW = this.opts.wmax*Math.pow(Math.E, -tempV*tempV/(2*this.opts.gaussian*this.opts.gaussian))*Math.pow(Math.E, -5*tempAcc);
          ...
          //Draw the segment between xy3 and xy4, this helps the sharp ending !!!
					...
					for(var u=0; u<sampleNumber; u++){
						...
						tempV = (1-t)*(1-t)*(1-t)*v3
										...
										+t*t*(1-t)*3*b3.v
										+t*t*t*v4;
						tempAcc = (1-t)*(1-t)*(1-t)*acc3
									+t*(1-t)*(1-t)*3*a2.acc
									+t*t*(1-t)*3*b2.acc
									+t*t*t*acc3;
						// tempW = this.opts.wmax*Math.pow(Math.E, -tempV*tempV/(2*this.opts.gaussian*this.opts.gaussian));
						tempW = this.opts.wmax*Math.pow(Math.E, -tempV*tempV/(2*this.opts.gaussian*this.opts.gaussian))*Math.pow(Math.E, -1*tempAcc);
						if(tempW<this.opts.wmin){
							tempW = 2
						}
						if(tempW>this.opts.wmax){
		...
		initControls: function() {
			this.controls = {};
			if(!this.opts.controls.length || !BezierChar.Control) return false;
			for(var i = 0; i < this.opts.controls.length; i++) {
				var c = null;
				if(typeof this.opts.controls[i] == "string") {
					c = new window['BezierChar']['Control'][this.opts.controls[i]](this);
				}else if(typeof this.opts.controls[i] == "object"){
					for(var controlName in this.opts.controls[i]) break;
					c = new window['BezierChar']['Control'][controlName](this, this.opts.controls[i][controlName]);
				}
				if (c) {
	...
	BezierChar.Control = function(drawingBoard, opts){
		this. board = drawingBoard;
		this.opts = _.extend({}, this.defaults, opts);
		this.el = document.createElement('div');
		this.el.classList.add('BezierChar-control');
		if(this.name){
			this.el.classList.add('BezierChar-control-'+this.name+'s');
		}
		this.initialize.apply(this, arguments);
		// return this;
	};

	BezierChar.Control.prototype = {
		name: '',
	...
	BezierChar.Control.extend = function(protoProps, staticProps){
		var parent = this;
		var child;
	...
	BezierChar.Control.DisplayMode = BezierChar.Control.extend({
		name: 'displaymode',
		...
		initialize: function() {
			...
			["brush", "skeleton", "strokeColor", "animation", "uniformWidth"].forEach(function(element, index, array){
				if(element) {
					tpl = tpl + '<button class="BezierChar-control-displaymode control-displaymode-'+element+'-button" data-mode="'+ element +'">'+ element+'</button> ';
				}
			});
			this.el.innerHTML = tpl;
	...
	BezierChar.Control.Navigation = BezierChar.Control.extend({
		name: 'navigation',
		...
		initialize: function(){
			...
			if(this.opts.imgRadio) {
				tplRadio = tplRadio + '<div class="BezierChar-navigation-radio"><button class="BezierChar-navigation-radio-blank" data-radio="blank">blank</button><button class="BezierChar-navigation-radio-grid" data-radio="grid">grid</button></div>';
				div.innerHTML = tplRadio;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.reset) {
				tplReset = tplReset + '<button class="BezierChar-control-navigation-reset">&times;</button>';
				div.innerHTML = tplReset;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.add) {
				tplAdd = tplAdd + '<button class="BezierChar-control-navigation-add">addChar</button>';
				div.innerHTML = tplAdd;
				this.el.appendChild(div.firstChild);
			};
			if(this.opts.update) {
				tplUpdate = tplUpdate + '<button class="BezierChar-control-navigation-update">update</button>';
			...
			// });
			this.el.getElementsByClassName("BezierChar-control-navigation-reset")[0].addEventListener('click', function(){
	...
	BezierChar.Control.Size = BezierChar.Control.extend({
		name: 'size',
		...
		initialize: function() {
			...
			// }
			var buttonArray = Array.prototype.slice.call(this.el.querySelectorAll('.BezierChar-control-size input'),0);
		...
		_rangeTemplate: function(){
			...
			["wmax", "wmin", "timeScale"].forEach(function(element, index, array){
				if(element) {
					tpl = tpl + '<div class="BezierChar-control-size BezierChar-control-size-'+ element +'" >'
					+'<div class="BezierChar-control-inner" title="'+ that.opts[element].size +'">'
					+'<span>'+element+'</span>' 
					+'<input type="range" min="'+ that.opts[element].min +'" max="'+ that.opts[element].max +'" value="'+ that.opts[element].size 
					+'" step="'+that.opts[element].step+'" class="BezierChar-control-size-range-input" data-range="'+ element +'">' 
					+ '<span class="BezierChar-control-size-range-current"></span>' 
					+'</div></div>'
				}
			});
			this.el.innerHTML = tpl;
		...
		_dropdownTemplet: function(){
			...
			["gaussian", "wmax", "wmin", "timeScale"].forEach(function(element, index, array){
				if(element) {
					tpl = tpl + '<div class="BezierChar-control-size-'+ element +'">'
					+'<div class="BezierChar-control-inner" title="'+ that.opts[element].size +'">' 
					+'<input type="dropdown" min="'+ that.opts[element].min +'" max="'+ that.opts[element].max +'" value="'+ that.opts[element].size 
					+'" step="'+that.opts[element].step+'" class="BezierChar-control-size-dropdown-input">' 
					+ '<span class="BezierChar-control-size-dropdown-current"></span>' 
					+'</div></div>'
				}
			});
			this.el.innerHTML = tpl;
	...
	BezierChar.Control.Curve = BezierChar.Control.extend({
		name : 'Curve',
...
// BezierChar.Control.File = BezierChar.Control.extend({
// 	name: "File",
// 	default: {
